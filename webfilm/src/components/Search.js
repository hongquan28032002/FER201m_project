...
function Search() {
  ...
  const filter = (name) => {
    name.includes("all")
      ...
      : setFilteredFilm(allfilms.filter((p) => p.category.includes(name)));
  ...
  return (
    <div>
      <div class="row justify-content-center mt-3 mb-2">
        <h2
          class="btn btn-outline-success border-success rounded-circle mr-3"
          onClick={() => filter("all")}
        >
          All
        </h2>
        ...
        {CatenameID.map((category) => (
          <h2
            class="btn btn-outline-success border-success rounded-circle mr-3 ml-3"
      ...
      <div class="container card mb-5 mt-2">
        <ul class="mt-3" style={{ textDecoration: "none", fontSize: "7px" }}>
          <div class="row">
            {filteredFilm.map((p) => (
              <div
                class="col-3 border-3  mb-5"
                style={{ width: "150x", height: "400px" }}
              >
                <li
                  class="card btn"
                  onClick={() => navigate(`/detail/${p.category}/${p.id}`)}
                >
                  <img
